...
/all_orders — Все активные заказы
...
async def cmd_all_orders(message: types.Message):
    """Show all active orders (not completed, cancelled, or hidden)"""
    if not await is_admin(message.from_user.id):
        await message.answer("⛔️ Доступ запрещён. Вы не администратор.")
        return
    ...
    if not orders:
        await message.answer("✅ Все заказы завершены или скрыты!")
        return
    ...
    for order in orders:
        if order.status in status_groups:
            status_groups[order.status].append(order)
    
    # Status display names and emojis
    status_display = {
        OrderStatus.NEW: "🆕 Новые заказы",
        OrderStatus.ACCEPTED: "✅ Принятые заказы",
        OrderStatus.IN_PROGRESS: "▶️ В работе",
        OrderStatus.READY: "🍽 Готовые к выдаче"
    }
    
    # Show header with count
    total_count = len(orders)
    await message.answer(
        f"📋 Все активные заказы: {total_count}\n\n"
        f"💡 Заказы, которые ещё в работе или не завершены"
    )
    
    # Display orders grouped by status
    for status in [OrderStatus.NEW, OrderStatus.ACCEPTED, OrderStatus.IN_PROGRESS, OrderStatus.READY]:
        orders_in_status = status_groups[status]
        if orders_in_status:
            await message.answer(f"\n{status_display[status]} ({len(orders_in_status)}):")
...
async def cmd_help(message: types.Message):
    ...
    help_text = """📖 **Справка по командам администратора**

━━━━━━━━━━━━━━━━━━━
**📋 УПРАВЛЕНИЕ ЗАКАЗАМИ**

**/new_orders** — Новые заказы (только со статусом "новый")
• Показывает заказы, требующие первичной обработки
• Заказы с оплатой картой выделены отдельно (⚠️)
• Кнопки: "Открыть" и "Отменить"

**/all_orders** — Все активные заказы
• Показывает ВСЕ заказы в работе (новые, принятые, готовятся, готовые)
• Группировка по статусам для удобства
• Помогает не пропустить заказы, которые ушли вверх в чате
• Не показывает завершённые, отменённые или скрытые

**/order_<ID>** — Детальная информация о заказе
• Например: /order_691cae998ef67346b9b3e5cd

━━━━━━━━━━━━━━━━━━━
**🔄 РАБОТА С ЗАКАЗАМИ**

**Заказы с оплатой картой (💳):**
1. Клиент указывает сумму перевода
2. Проверьте поступление оплаты (до 10 минут)
3. "✅ Подтвердить оплату" → заказ переходит в accepted
4. "❌ Отклонить оплату" → payment_failed

**Обычные заказы (💵):**
• new → accepted (принять)
• accepted → in_progress (в работу)
• in_progress → ready (готово)
• ready → completed (завершить)

**На любом этапе:**
• Можно отменить заказ (✖️)
• После завершения/отмены: кнопка "🙈 Скрыть"

━━━━━━━━━━━━━━━━━━━
**📦 УПРАВЛЕНИЕ ИНВЕНТАРЁМ**

**/inventory** — Управление доступностью товаров
• Включить/выключить позиции меню
• Изменения сразу видны в клиентском боте
• Отключённые товары клиенты не могут заказать
...
• Итоги за неделю
• Выручка
• Топ-3 популярных позиций

**/stats_orders [period]** — Сводка по заказам
• Примеры: /stats_orders week, /stats_orders today
• По умолчанию: week (неделя)

**/earnings [period]** — Выручка за период
• Примеры: /earnings month, /earnings today
• По умолчанию: week (неделя)
...
**/broadcast <текст>** — Рассылка всем администраторам
• Пример: /broadcast Сегодня закрываемся на час раньше

━━━━━━━━━━━━━━━━━━━
**📊 СТАТУСЫ ЗАКАЗОВ**

🆕 **new** — Новый заказ (требует обработки)
✅ **accepted** — Принят (оплата подтверждена)
▶️ **in_progress** — В работе (готовится)
🍽 **ready** — Готов к выдаче/доставке
✔️ **completed** — Успешно завершён
✖️ **cancelled** — Отменён
...
• Используйте /all_orders чтобы проверить, не пропустили ли заказы
• Заказы с картой всегда проверяйте в течение 10 минут
• Скрывайте завершённые заказы для чистоты чата
• Все времена показаны в узбекском часовом поясе (UTC+5)"""
